...
// ---------- 補助関数 ----------
// HTML エスケープ。ポップアップ等は innerHTML で描画するため、データ由来の文字列
// （モスク名は第三者マイマップ由来、首長データはエージェント＋web収集由来など、いずれも
// 完全には信頼できない）を埋め込む前に必ず通し、格納型XSSを防ぐ。数値・自作HTMLは通さない。
function esc(s){
  return String(s==null?'':s).replace(/[&<>"']/g, c=>(
    {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
}
// href に入れる URL は http/https のみ許可（javascript: 等のスキームを弾く）。不許可なら空。
function safeUrl(u){
  u = String(u==null?'':u).trim();
  return /^https?:\/\//i.test(u) ? u : '';
}
// 出典リンクの共通生成（許可URLのときだけ <a> を返す）
...
function buildPoints(){
  ...
  mosquesGeo.features.forEach(f=>{
    ...
    // ポップアップ HTML（名称・種別・コード・出典リンク）。データ由来はすべて esc/safeUrl 経由。
    let html = `<span class="pname">${removed?'<span class="removed">★ </span>':''}${esc(p.name||'（名称不明）')}</span>`;
    html += `<div class="pcounts">${LEVEL_LABEL_POINT(cat)}${p.code?` · ${esc(p.code)}`:''}</div>`;
    const psrc = safeUrl(p.source_url);
    if(psrc) html += `<a href="${esc(psrc)}" target="_blank" rel="noopener">出典 ↗</a>`;
    m.bindPopup(html);
    cats[cat].push(m);
  });
...
function partyTag(name){
  ...
  // 色は #RRGGBB 形式のみ許可（style 属性への注入を防ぐ）。表示テキストは esc。
  const color = /^#[0-9a-fA-F]{3,8}$/.test(p.color||'') ? p.color : '#9aa0a6';
  return `<span class="ptag" style="background:${color}">${esc(p.short||name)}</span>`;
}
const shortParty = n => (PARTIES[n]||{}).short || n;

// 単一議員（知事・衆院）用の共通ラッパ（body は呼び出し側が esc/partyTag 済みのHTML）
function repWrap(office, body, sourceUrl, asOf){
  return `<div class="reps"><div class="rep-office">${esc(office)}</div>`+
         `<div class="rep-name">${body}</div>`+
         `<div class="rep-src">${esc(asOf||'')}現在${srcLink(sourceUrl,'出典↗')}</div></div>`;
}
...
function repsBlock(p){
  if(p.level==='pref' && REPS.pref){
    ...
    const win = r.winCount ? `（${esc(r.winCount)}期）` : '';
    const endorse = (r.endorsers && r.endorsers.length)
      ? `<span class="rep-endorse">${r.endorsers.map(x=>esc(shortParty(x))).join('・')} 推薦</span>` : '';
    return repWrap(REPS.pref.office, `${esc(r.name)}${win} ${partyTag(r.affiliation)}${endorse}`, r.sourceUrl, REPS.pref.asOf);
  }
  if(p.level==='hr' && REPS.hr){
    const r=REPS.hr.records[p.code]; if(!r) return '';
    const win = r.winCount ? `（${esc(r.winCount)}期）` : '';
    const kaiha = (r.kaiha && r.kaiha!==r.party) ? `<span class="rep-endorse">会派: ${esc(shortParty(r.kaiha))}</span>` : '';
    return repWrap(REPS.hr.office, `${esc(r.name)}${win} ${partyTag(r.party)}${kaiha}`, r.sourceUrl, REPS.hr.asOf);
  }
  if(p.level==='hc' && REPS.hc){
    const r=REPS.hc.records[p.code]; if(!r || !r.members) return '';
    const rows = r.members.map(m=>
      (m.vacant || !m.name)
        ? `<div class="rep-mem rep-vacant">（欠員）<span class="rep-cls">${m.cls?`任期~${esc(m.cls)}`:''}</span>${m.note?`<span class="rep-note"> ${esc(m.note)}</span>`:''}</div>`
        : `<div class="rep-mem">${esc(m.name)} ${partyTag(m.party)}<span class="rep-cls">${m.cls?`任期~${esc(m.cls)}`:''}</span></div>`).join('');
    const filled = r.members.filter(m=>!(m.vacant || !m.name)).length;
    const vac = r.members.length - filled;
    const mag = vac ? `${r.members.length}議席・${vac}欠員` : `${r.members.length}名`;
    return `<div class="reps"><div class="rep-office">${esc(REPS.hc.office)} <span class="rep-mag">（${mag}）</span></div>`+
           `${rows}<div class="rep-src">${esc(REPS.hc.asOf||'')}現在</div></div>`;
  }
  if(p.level==='muni' && REPS.muni){
    const r=REPS.muni.records[p.code]; if(!r) return '';
    // 首長不在（北方領土など）は「—」と注記のみ
    if(r.noHead) return `<div class="reps"><div class="rep-office">${esc(r.office||'首長')}</div>`+
      `<div class="rep-name">—</div><div class="rep-src">${esc(r.note||'')}</div></div>`;
    const term = r.termEnd ? `<span class="rep-cls">任期満了 ${esc(r.termEnd)}</span>` : '';
    // 政令市の行政区は親市の市長を表示し、任命職である旨を注記
    const inh = r.inherited ? `<div class="rep-endorse">${esc(r.parentCity)}長（行政区長は任命職）</div>` : '';
    return `<div class="reps"><div class="rep-office">${esc(r.office||'首長')}</div>`+
           `<div class="rep-name">${esc(r.name)} ${term}</div>${inh}`+
           `<div class="rep-src">${esc(REPS.muni.asOf||'')}現在${srcLink(r.sourceUrl,'出典↗')}</div></div>`;
  }
  return '';
}
...
function districtPopup(p){
  ...
  return `<span class="pname">${esc(p.name||'（名称不明）')}</span>`+
    `<div class="pcounts">${esc(LEVEL_LABEL[p.level]||p.level)}${p.code?` · ${esc(p.code)}`:''}<br>`+
...
function attachDistrictHandlers(layer, isFilled){
  layer.on('mouseover', e=>{
    ...
    hb.innerHTML=`<b>${esc(p.name)}</b> — 🕌 ${p.mosque||0} · 計 ${metricValue(p)}`;
    document.getElementById('readout').innerHTML =
      `<b>${esc(p.name)}</b><br>🕌 ${p.mosque||0} モスク · 🧎 ${p.prayer||0} 祈祷室 · 🏗️ ${p.planned||0} 予定地`;
  });
  layer.on('mouseout', e=>{
...
function renderRanking(){
  ...
  if(rows.length===0){
    ...
    rows.forEach((f,i)=>{
      const p=f.properties;
      html += `<tr data-code="${esc(p.code)}"><td class="rk-num">${i+1}</td>`+
        `<td class="rk-name" title="${esc(p.name||'')}">${esc(p.name||'')}</td>`+
